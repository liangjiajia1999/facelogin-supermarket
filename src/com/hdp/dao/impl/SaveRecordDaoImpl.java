...
public class SaveRecordDaoImpl implements SaveRecordDao {
	...
	public List<SaveRecord> queryFromTo(String start, String end) {
		...
		String sql="select * from saverecord where stime> ? and stime< ?";
	...
	public int add(SaveRecord obj) {
		// TODO Auto-generated method stub
		int i=0;
		String sql="insert into saverecord(vid,stime,smoney,uid)values(?,?,?,?)";
		Connection conn=DBUtil.getConnection();
		PreparedStatement pstmt=null;
		try {
			pstmt=conn.prepareStatement(sql);
	...
	public int deleteToUser(String user) {
		// TODO Auto-generated method stub
		int i=0;
		String sql="delete from saverecord where uid=?";
		Connection conn=DBUtil.getConnection();
		PreparedStatement pstmt=null;
		try {
			pstmt=conn.prepareStatement(sql);
	...
	public int deleteToVip(String vip) {
		// TODO Auto-generated method stub
		int i=0;
		String sql="delete from saverecord where vid=?";
		Connection conn=DBUtil.getConnection();
		PreparedStatement pstmt=null;
		try {
			pstmt=conn.prepareStatement(sql);
